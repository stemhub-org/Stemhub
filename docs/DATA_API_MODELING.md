...
| `/pull-requests/{id}/close` | `POST` | `id` | Close without merging (`OPEN` → `CLOSED`; `409` if not open). `CLOSED` is terminal — users open a new PR instead of reopening. Merge is a separate, future endpoint. |
...
- Both `CLOSED` and `MERGED` are terminal (SPECIFICATION.md §7, §19). A closed PR is not reopened — users open a new PR. Any other transition answers `409`.
